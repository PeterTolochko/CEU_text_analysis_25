...
# Few-shot examples
few_shot_examples <- "
Classify the following political statements as liberal or conservative:

Statement: Support for LGBTQ+ rights and marriage equality.
Label: Liberal

Statement: Advocacy for universal healthcare.
Label: Liberal

Statement: Promotion of renewable energy and climate action.
Label: Liberal

Statement: Emphasis on traditional family values and marriage.
Label: Conservative

Statement: Support for lower taxes and smaller government.
Label: Conservative

Statement: Defense of Second Amendment rights.
Label: Conservative
"
...
# Query the model using few-shot prompting
for (statement in additional_statements) {
  prompt <- paste0(few_shot_examples, "\n\nStatement: ", statement, "\nLabel:")
  print(query(prompt))
}
...
for (i in 1:nrow(small_test)) {
  ...
  result <- query(concat, model = "llama2")
